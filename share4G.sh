...
# configuracion de conexion compartida de 4G a Ethernet - conexion a la web con 4G
...
mv /etc/dnsmasq.conf /etc/dnsmasq.conf.4G
...
# Configurar NAT
iptables -t nat -A POSTROUTING -o wwan0 -j MASQUERADE  && sudo iptables -A FORWARD -i wwan0 -o eth0 -m state --state RELATED,ESTABLISHED -j ACCEPT && sudo iptables -A FORWARD -i eth0 -o wwan0 -j ACCEPT
